...
export default class TrialService {
  ...
  getServiceModel(callback) {
    this.DSUStorage.call("listDSUs", this.SERVICE_PATH, (err, dsuList) => {
      ...
      let getServiceDsu = (dsuItem) => {
        this.DSUStorage.getItem(
          this._getDsuStoragePath(dsuItem.identifier),
          (err, content) => {
            if (err) {
  ...
  updateTrial(data, callback) {
    ...
    this.DSUStorage.setObject(
      this._getDsuStoragePath(data.uid),
      data,
      (err) => {
        if (err) {
          callback(err, undefined);
  ...
  unmountTrial(trialId, callback) {
    let unmountPath = this.SERVICE_PATH + "/" + trialId;
